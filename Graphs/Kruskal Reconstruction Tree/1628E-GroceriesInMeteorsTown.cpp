// use sparse-table lca instead of this hld one.
...
struct lazy_segment_tree{
  ...
  pair<int, int> range_minmax(int L, int R, int i, int l, int r){
    ...
    if (r <= L || R <= l){
      ...
      return make_pair(on_mn[i], on_mx[i]);
    } else {
      int m = (l + r) / 2;
  ...
  pair<int, int> range_minmax(int L, int R){
    return range_minmax(L, R, 0, 0, N);
  }
};
...
int main(){
  ...
  for (int i = 0; i < q; i++){
    ...
    if (t == 3){
      ...
      cin >> x;
      x--;
      pair<int, int> R = ST.range_minmax(0, n);
      if (R.first == INF){
        cout << -1 << "\n";
      } else {
